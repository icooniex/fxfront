...
## Problem
Bot connections were being reset during SSL handshake:
```
...
1. **Commit and push changes:**
   ```bash
   git add gunicorn.conf.py fxfront/settings.py Procfile
   git commit -m "Fix: Add Gunicorn config to prevent SSL connection resets"
